...
- 🎓 **LTI 1.3 Tool Provider + LTI Advantage** — launch students straight onto a desktop from **Canvas, Schoology, Moodle, Blackboard or D2L** with no join code. Full OIDC handshake (`id_token` verified against the platform JWKS, nonce/message-type/version/deployment enforcement, JIT provisioning). Plus the **two-way Advantage** services: **NRPS** pulls the LMS course roster into the class (idempotent, de-provisioning), and **AGS** pushes graded scores back into the LMS gradebook (durable, retried, dead-lettered). Tool URLs + platform registration in the admin console; keys at `/api/lti/jwks.json`.
