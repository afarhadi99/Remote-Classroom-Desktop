...
export async function POST(req: Request) {
  ...
  if (!parsed.success) return apiError('Provide a valid class name and settings.')
  ...
  if (!isUnlimited(plan.maxClasses) && classCount >= plan.maxClasses) {
    return apiError(`Your ${plan.name} plan includes ${plan.maxClasses} class(es). Upgrade for more.`, 402)
  }
  ...
  await logEvent({
    ...
    type: 'provision_all',
    actorRole: 'system',
    message: `Class created via API key`,
  })
  return json(
    { id: classroom.id, name: classroom.name, joinCode: classroom.joinCode },
    201,
  )
}
